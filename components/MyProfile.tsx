...
interface ActivityLog {
  id: string;
  type: 'profile_view' | 'profile_update' | 'password_reset' | 'email_verification_sent';
  timestamp: Date;
  details?: string;
}
...
const MyProfile: React.FC<MyProfileProps> = ({ 
  ...
  const logActivity = async (memberId: string, type: ActivityLog['type'], details?: string) => {
    try {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      setIsSaving(true);
      await onUpdateMember(myMember.id, editableMember);
      await logActivity(myMember.id, 'profile_update', 'Updated profile information');
      setIsEditing(false);
      // Refresh member data
      const updatedMember = { ...myMember, ...editableMember };
      setMyMember(updatedMember);
      calculateProfileCompletion(updatedMember);
    } catch (error) {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      ...
      if (myMember) {
        await logActivity(myMember.id, 'password_reset', 'Password reset email sent');
      }
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  To link your account to a member profile, please contact your church administrator with your email address: 
